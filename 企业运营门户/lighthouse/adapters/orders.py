"""Original order side-screen calculations. No BOM, MES or project management routes."""
import re
from datetime import datetime, timedelta, timezone
LOCAL_TZ = timezone(timedelta(hours=8))

def match_production(rows, order_name, product_id):
    if not order_name or not product_id:
        return None
    matches = [row for row in rows if rel_id(row.get('product_id')) == product_id
               and order_name in re.split(r'[,;，；\s]+', str(row.get('origin') or ''))]
...
def build(client):
    ...
    orderpoint_rows = client.search_read_all(
    ...
    for line in sale_lines:
        ...
        mrp = match_production(mrp_rows, order.get('name'), rel_id(line.get('product_id')))
        # A product-level reorder rule is not proof that this order is short.
        # Preserve overdue priority; only an exact order origin may label production.
        if status != '已逾期' and remaining > 0 and mrp:
            status = "生产中" if mrp.get("state") == "progress" else "已规划"
        remark = "产品补货 " + qty_text(need_qty) + '（非订单缺口）' if need_qty > 0 else 'Odoo待交付'
        updated_at = max_dt(
            line.get("write_date"),
        ...
        delivery_rows.append(
            {
                "id": line['id'],
                "orderId": rel_id(line.get('order_id')),
                ...
                "stages": [],  # No fabricated production route or progress.
