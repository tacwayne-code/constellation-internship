...
function renderOrders() {
  ...
  const pages = Math.max(1, Math.ceil(rows.length / visibleOrderCount));
  currentPage = Math.min(currentPage, pages - 1);
  const visible = rows.slice(currentPage * visibleOrderCount, (currentPage + 1) * visibleOrderCount);
  const controls = document.querySelector('#orderPages');
  if(controls){controls.hidden=pages===1;document.querySelector('#pageLabel').textContent=`${currentPage+1} / ${pages}`;document.querySelector('#prevPage').disabled=currentPage===0;document.querySelector('#nextPage').disabled=currentPage===pages-1;}
  ...
  $("#orderList").innerHTML = `
    ...
    ${visible.map((row, index) => `
      <article class="order-card delivery-row page-enter ${esc(row.priority || "running")}" style="--order-accent:${colorMap[row.priority] || colorMap.running};animation-delay:${index * 55}ms">
        ...
        <div class="order-field due-field"><span>${esc(row.delivery)}</span><small>只读快照</small></div>
      </article>
    `).join("")}
  `;
}
...
function renderBottom() {
  ...
  const hiddenRows = (dashboard?.deliveryRows || []).filter((_,i)=>i<currentPage*visibleOrderCount||i>=(currentPage+1)*visibleOrderCount);
...
async function loadData() {
  const payload = await window.Lighthouse.read('orders');
  if (payload?.updatedAt && payload.updatedAt === renderedSnapshotAt) return;
  if (payload?.ok && payload.data) {
    dashboard = payload.data;
    renderKpis(); renderOrders(); renderBottom(); updateBroadcast();
    renderedSnapshotAt = payload.updatedAt;
    setText('#refreshTime', new Date(payload.updatedAt).toLocaleTimeString('zh-CN', {hour12:false}));
...
window.Lighthouse.onRefresh = loadData;
document.querySelector('#prevPage').onclick=()=>{currentPage=Math.max(0,currentPage-1);renderOrders();renderBottom();};
document.querySelector('#nextPage').onclick=()=>{currentPage++;renderOrders();renderBottom();};
