...
let currentScreen = new URLSearchParams(location.search).get("screen") === "inventory" ? "inventory" : "purchase";
let levelFilter = "all";
let searchTerm = "";
let expandedRiskId = "";
let selectedRiskId = "";
...
function readDismissedReplenishments() { return new Set(); }
...
function readDismissedEvents() { return new Set(); }
...
async function loadRealDashboard() {
  const payload = await window.Lighthouse.read('inventory');
...
async function loadReplenishmentsOnly() { return false; }
