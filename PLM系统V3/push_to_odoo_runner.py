...
try:
    ...
    log.info(f'Calling authenticate to {base} db={cfg[1]} user={cfg[2]} pwd_len={len(api_password)}')
    try:
        uid = common.authenticate(cfg[1], cfg[2], api_password, {})
    except Exception as auth_exc:
        log.error(f'Authenticate RPC raised: {type(auth_exc).__name__}: {auth_exc}')
        con.close()
        fail(f'无法连接 Odoo ({type(auth_exc).__name__}: {str(auth_exc)[:120]})')
        sys.exit(1)
    log.info(f'auth uid={uid}')
    if not uid:
        # Diagnose the most common cause so the operator can fix it.
        try:
            server_version = common.version()
            version_str = server_version.get('server_version', '?')
        except Exception:
            version_str = '无法连接'
        try:
            db_proxy = xmlrpc.client.ServerProxy(f'{base}/xmlrpc/2/db', context=ctx, allow_none=True)
            dbs = db_proxy.list()
            db_match = '已找到' if cfg[1] in dbs else '未在列表中'
        except Exception as db_exc:
            dbs, db_match = [], f'无法读取({db_exc})'
        detail = (
            f'用户 {cfg[2]!r} 在数据库 {cfg[1]!r} (服务器 {version_str}, db-list {db_match}) '
            f'认证失败：账号/密码错误或该用户无登录权限。'
        )
        log.error(detail)
        con.close()
        fail(detail)
